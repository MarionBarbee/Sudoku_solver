...
auto gseed = chrono::duration_cast<chrono::microseconds>(chrono::steady_clock::now().time_since_epoch()).count();
...
void clearbtable();
void reinsertglist();
int  insg2table(int r, int c, int v);
void clearg2table();
void  insb2table(int glindex);
...
int  insABlist(int r, int c, int v);
void delABlist(int listptr);
void printbadlist();
void cleargtable();
int forcerror();
int solveABlist();
...
int collectpairsrow();
int collectpairscol();
int insAlistrow(int r, int c, int v);
int insBlistrow(int r, int c, int v);
int insAlistcol(int r, int c, int v);
int insBlistcol(int r, int c, int v);
void printAlistrow();
void printBlistrow();
void printAlistcol();
void printBlistcol();
...
void delAlistrow(int listptr);
void delBlistrow(int listptr);
void delAlistcol(int listptr);
void delBlistcol(int listptr);
...
//===================================================================================
int procrowlists(){
	gseed = chrono::duration_cast<chrono::microseconds>(chrono::steady_clock::now().time_since_epoch()).count();
	mt19937_64 generator(gseed);
	int const A = 1;
	int res = 0;
	int randomAorBlistpick = generator() % 2 + 1;
	if (randomAorBlistpick == A){
		 res = pushAlistrow();
	}
	else{ res = pushBlistrow(); }
	return res;
}
//===================================================================================
int proccollists(){
...
int pushAlistrow(){
	...
	int r, c, v, res, randptr;
    randptr = generator() % ptrArow + 1;
	r = Alistrow[randptr].r;
	c = Alistrow[randptr].c;
	v = Alistrow[randptr].v;
	thisisaguess = true;
	glastwrite = "==============pushAlistrow=================";
	res = inspuzzle(r, c, v);
	thisisaguess = false;
	if ((res > 0) && (!glerr)){
		delAlistrow(randptr);
		if (debug){ printAlistrow(); }
	}
	return res;
}
//===================================================================================
int pushBlistrow(){
	gseed = chrono::duration_cast<chrono::microseconds>(chrono::steady_clock::now().time_since_epoch()).count();
	mt19937_64 generator(gseed);
	if (ptrBrow == 0){ return 0; }
	int r, c, v, res, randptr;
	randptr = generator() % ptrBrow + 1;
	r = Blistrow[randptr].r;
	c = Blistrow[randptr].c;
	v = Blistrow[randptr].v;
	thisisaguess = true;
	glastwrite = "==============pushBistrow=================";
	res = inspuzzle(r, c, v);
	thisisaguess = false;
	if ((res > 0) && (!glerr)){
		delBlistrow(randptr);
		if (debug){ printBlistrow(); }
	}
	return res;
}
//===================================================================================
int pushAlistcol(){
	gseed = chrono::duration_cast<chrono::microseconds>(chrono::steady_clock::now().time_since_epoch()).count();
	mt19937_64 generator(gseed);
	if (ptrAcol == 0){ return 0; }
	int r, c, v, res, randptr;
	randptr = generator() % ptrAcol + 1;
	r = Alistcol[randptr].r;
	c = Alistcol[randptr].c;
	v = Alistcol[randptr].v;
	res = inspuzzle(r, c, v);
	thisisaguess = true;
	glastwrite = "==============pushAlistcol==========";
	res = inspuzzle(r, c, v);
	thisisaguess = false;
	if ((res > 0)&&(!glerr)){
		delAlistcol(randptr);
		if (debug){ printAlistcol(); }
	}
	return res;
}
//===================================================================================
int pushBlistcol(){
	gseed = chrono::duration_cast<chrono::microseconds>(chrono::steady_clock::now().time_since_epoch()).count();
	mt19937_64 generator(gseed);
	if (ptrBcol == 0){ return 0; }
	int r, c, v, res, randptr;
	randptr = generator() % ptrBcol + 1;
	r = Blistcol[randptr].r;
	c = Blistcol[randptr].c;
	v = Blistcol[randptr].v;
	thisisaguess = true;
	glastwrite = "==============pushBistcol==============";
	res = inspuzzle(r, c, v);
	thisisaguess = false;
	if ((res > 0)&&(!glerr)){
		delBlistcol(randptr);
		if (debug){ printBlistcol(); }
	}
	return res;
}
//===================================================================================
...
int solveABlist(){
	...
	if (ABptr == 1){
		//it is bad.  mate is good.insert it and get out.
		r = ABlist[1].r;
		c = ABlist[1].c;
		v = ABlist[1].v;
		insbadlist(r, c, v);
	...
	int groupsize;
	int rem = ABptr % 2;
	if (rem == 0){ groupsize = ABptr / 2; }
	else{ groupsize = ABptr + 1 / 2; }
	int res[100];
	gtype grouplist[100];
	int found = 0;
	//how many groups of size AB/2 of AB single digits?
	stackptr = 0;
	popp();
	//popp(0) results in restore of known correct base puzzle
	cleargtable();
	while (true){
		if (debug){
			printpuzzle();
			cout << "collecting groups for insertion" << endl;
			cout << "there is a group of size ABptr/2 which are all correct" << endl;
			//need spuzzle print
			cout << "groupsize= " << groupsize << endl;
		}
		for (int i = 1; i <= groupsize; i++){
			while (true){
				randptr = generator() % ABptr + 1;
				rem = randptr % 2;
				if (rem == 0){//even look  for -1
					r = ABlist[randptr - 1].r;
					c = ABlist[randptr - 1].c;
					v = ABlist[randptr - 1].v;
				}
				else{
					r = ABlist[randptr + 1].r;
					c = ABlist[randptr + 1].c;
				...
				found = findinglist(r, c, v);
				if (!found){//check current
					r = ABlist[randptr].r;
					c = ABlist[randptr].c;
					v = ABlist[randptr].v;
					found = findinglist(r, c, v);
					if (!found){ break; }  //valid
				}
			}//while
			if (debug){
				cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!randptr=====" << randptr << endl;
			}
			int r = ABlist[randptr].r;
			int c = ABlist[randptr].c;
			int v = ABlist[randptr].v;
			glastwrite = "__________________________group ABptr/2_________________________";
			thisisaguess = false;
			if (debug){
				cout << "res[" << i << "]=inspuzzle(" << r << "," << c << "," << v << ");" << endl;
			}
			res[i] = inspuzzle(r, c, v);
			if (res[i] > 0){//insert good. get another
				insgtable(r, c, v);
				gptr++;
				if (debug){ cout << "#inserted  of group= " << i << endl; }
				grouplist[gptr].r = r;
				grouplist[gptr].c = c;
				grouplist[gptr].v = v;
				if (debug){ cout << "res[" << i << "]=" << res[i] << endl; }
			}
			else{ glerr = false; }
		}//for
		//now all 3 inserted group x
		int errortype = forcerror();
		if (errortype != none){
			stackptr = 0;
			popp();
		}//keep trying
...
bool popp(){
	...
	int num = 0;
	zcnt = 0;
	if (debug){ cout << "popping  stackptr=" << stackptr << endl; }
	if (stackptr > 0){ stackptr--; }
	if (debug){ cout << "popped stack new  stackptr=" << stackptr << endl; }
...
void delgtable(int glindex){
	...
	while (glindex < top){
		if (top > 1){
			glist[glindex].r = glist[glindex + 1].r;
			glist[glindex].c = glist[glindex + 1].c;
			glist[glindex].v = glist[glindex + 1].v;
			r = glist[glindex].r;
			c = glist[glindex].c;
			v = glist[glindex].v;
			gtable[r][c] = v;
...
int finishrow(int r){
	...
	//	thisisaguess = false;
		...
		int c1 = blrow[1];
		int c2 = blrow[2];
		glastwrite = "&&&&&&&&&&&&&&&&&&&&&finishrow1&&&&&&&&&&&&&&&&&&&&&&&&&&";
		res = inspuzzle(r, c1, m1);
		glastwrite = "&&&&&&&&&&&&&&&&&&&&&finishrow2&&&&&&&&&&&&&&&&&&&&&&&&&&";
		res = inspuzzle(r, c2, m2);
		if (res > 0){ thisisaguess = false; procpuzzle(); }
		if ((glerr)||(res==0)){
			rldsaved();
			//glerr = false;
			glastwrite = "&&&&&&&&&&&&&&&&&&&&&finishrow3&&&&&&&&&&&&&&&&&&&&&&&&&&";
			res = inspuzzle(r, c2, m1);
			glastwrite = "&&&&&&&&&&&&&&&&&&&&&finishrow4&&&&&&&&&&&&&&&&&&&&&&&&&&";
			res = inspuzzle(r, c1, m2);
			if (res > 0){ thisisaguess = false; procpuzzle(); }
...
int gprocall(int r){
	...
	for (int i = 1; i <= mcount; ++i){
		...
		if ((elimcount == mcount - 1) && (targbl > 0)){
			...
			if (tboxres == 0){
				glastwrite = "**********************************************elim 1******************************gproc7855"; 
				result = inspuzzle(r, targbl, currentm[i]); return result;
...
int collectpairsrow(int runit, int num){
	...
	if ((rowunit[runit].nc[num] == 2) && (rowunit[runit].targrow[num] != 0) && (rowunit[runit].targbox[num] != 0)){
		...
		if (places == 2){
			...
			insAlistrow(r, chA, num);
			insBlistrow(r, chB, num);
		}
	}
	return 0;
}
...
int  insAlistrow(int r, int c, int v){
	++ptrArow;
	r = Alistrow[ptrArow].r = r;
	c = Alistrow[ptrArow].c = c;
	v = Alistrow[ptrArow].v = v;
	return 0;
}
//===================================================================================
//===================================================================================
int  insBlistrow(int r, int c, int v){
	++ptrBrow;
	r = Blistrow[ptrBrow].r = r;
	c = Blistrow[ptrBrow].c = c;
	v = Blistrow[ptrBrow].v = v;
	return 0;
}
//===============================================================================
//===============================================================================
void  printAlistrow(){
	...
	for (int i = 1; i <= ptrArow; i++){
		cout << "Alistrow[" << i << "].r = " << Alistrow[i].r << endl;
		cout << "Alistrow[" << i << "].c = " << Alistrow[i].c << endl;
		cout << "Alistrow[" << i << "].v = " << Alistrow[i].v << endl;
		cout << "=============================================="<<endl;
	}
	return;
}
...
void  printBlistrow(){
	...
	for (int i = 1; i <= ptrBrow; i++){
		cout << "Blistrow[" << i << "].r = " << Blistrow[i].r << endl;
		cout << "Blistrow[" << i << "].c = " << Blistrow[i].c << endl;
		cout << "Blistrow[" << i << "].v = " << Blistrow[i].v << endl;
		cout << "==============================================" << endl;
	}
	return;
}
...
void  printAlistcol(){
	...
	for (int i = 1; i <= ptrAcol; i++){
		cout << "Alistcol[" << i << "].r = " << Alistcol[i].r << endl;
		cout << "Alistcol[" << i << "].c = " << Alistcol[i].c << endl;
		cout << "Alistcol[" << i << "].v = " << Alistcol[i].v << endl;
		cout << "==============================================" << endl;
	}
	return;
}
...
void  printBlistcol(){
	...
	for (int i = 1; i <= ptrBcol; i++){
		cout << "Blistcol[" << i << "].r = " << Blistcol[i].r << endl;
		cout << "Blistcol[" << i << "].c = " << Blistcol[i].c << endl;
		cout << "Blistcol[" << i << "].v = " << Blistcol[i].v << endl;
		cout << "==============================================" << endl;
	}
	return;
}
...
int  insAlistcol(int r, int c, int v){
	++ptrAcol;
	r = Alistcol[ptrAcol].r = r;
	c = Alistcol[ptrAcol].c = c;
	v = Alistcol[ptrAcol].v = v;
	return 0;
}
//===================================================================================
//===================================================================================
int  insBlistcol(int r, int c, int v){
	++ptrBcol;
	r = Blistcol[ptrBcol].r = r;
	c = Blistcol[ptrBcol].c = c;
	v = Blistcol[ptrBcol].v = v;
	return 0;
}
//===============================================================================
...
void reinsertglist(){
	...
	int listptr = generator() % top + 1;

	int r = glist[listptr].r;
	int c = glist[listptr].c;
	int v = glist[listptr].v;
	...
	if (listptr == top){
		...
		while (true){
			...
			glist[listptr].r = glist[listptr + 1].r;
			glist[listptr].c = glist[listptr + 1].c;
			glist[listptr].v = glist[listptr + 1].v;
			glist[listptr].zcnt = glist[listptr + 1].zcnt;
			listptr++;
		}
	}
	gtable[r][c] = 0;
	...
	//else{ rldsaved(); lzcnt = zcnt; predictpath(); }
...
//==========================================================
void printABlist(){
	for (int i = 1; i <= ABptr; i++){
		cout << "ABlist[" << i << "].r=" << ABlist[i].r << endl;
		cout << "ABlist[" << i << "].c=" << ABlist[i].c << endl;
		cout << "ABlist[" << i << "].v=" << ABlist[i].v << endl;
		cout << "=======================================================" << endl;
	}
	return;
}
...
void printbadlist(){
	for (int i = 1; i <= blptr; i++){
		cout << "badlist[" << i << "].r=" << badlist[i].r << endl;
		cout << "badlist[" << i << "].c=" << badlist[i].c << endl;
		cout << "badlist[" << i << "].v=" << badlist[i].v << endl;
	//	cout << "badlist[" << i << "].zcnt=" << badlist[i].zcnt << endl;
		cout << "=======================================================" << endl;
	}
	return;
}
...
void delbadlist(int listptr){
	...
	badlist[listptr].v = 0;
	if (listptr < blptr){
		badlist[listptr].r = badlist[listptr + 1].r;
		badlist[listptr].c = badlist[listptr + 1].c;
		badlist[listptr].v = badlist[listptr + 1].v;
	}
	if (blptr > 0){ blptr--; }
	return;
}
...
void delABlist(int listptr){
	int r = ABlist[listptr].r;
	int c = ABlist[listptr].c;
	int v = ABlist[listptr].v;
	ABlist[listptr].r = 0;
	ABlist[listptr].c = 0;
	ABlist[listptr].v = 0;
	if (listptr < ABptr){
		ABlist[listptr].r = ABlist[listptr + 1].r;
		ABlist[listptr].c = ABlist[listptr + 1].c;
		ABlist[listptr].v = ABlist[listptr + 1].v;
	}
	if (ABptr > 0){ ABptr--; }
	return;
}
...
//==================================================================================================
void delAlistrow(int listptr){
	int r = Alistrow[listptr].r;
	int c = Alistrow[listptr].c;
	int v = Alistrow[listptr].v;
	Alistrow[listptr].r = 0;
	Alistrow[listptr].c = 0;
	Alistrow[listptr].v = 0;
	if (listptr < ptrArow){
		Alistrow[listptr].r = Alistrow[listptr + 1].r;
		Alistrow[listptr].c = Alistrow[listptr + 1].c;
		Alistrow[listptr].v = Alistrow[listptr + 1].v;
	}
...
//===========================================================================================
void delBlistrow(int listptr){
	int r = Blistrow[listptr].r;
	int c = Blistrow[listptr].c;
	int v = Blistrow[listptr].v;
	Blistrow[listptr].r = 0;
	Blistrow[listptr].c = 0;
	Blistrow[listptr].v = 0;
	if (listptr < ptrBrow){
		Blistrow[listptr].r = Blistrow[listptr + 1].r;
		Blistrow[listptr].c = Blistrow[listptr + 1].c;
		Blistrow[listptr].v = Blistrow[listptr + 1].v;
	}
...
//==================================================================================================
void delAlistcol(int listptr){
	int r = Alistcol[listptr].r;
	int c = Alistcol[listptr].c;
	int v = Alistcol[listptr].v;
	Alistcol[listptr].r = 0;
	Alistcol[listptr].c = 0;
	Alistcol[listptr].v = 0;
	if (listptr < ptrAcol){
		Alistcol[listptr].r = Alistcol[listptr + 1].r;
		Alistcol[listptr].c = Alistcol[listptr + 1].c;
		Alistcol[listptr].v = Alistcol[listptr + 1].v;
	}
...
//===========================================================================================
void delBlistcol(int listptr){
	int r = Blistcol[listptr].r;
	int c = Blistcol[listptr].c;
	int v = Blistcol[listptr].v;
	Blistcol[listptr].r = 0;
	Blistcol[listptr].c = 0;
	Blistcol[listptr].v = 0;
	if (listptr < ptrBcol){
		Blistcol[listptr].r = Blistcol[listptr + 1].r;
		Blistcol[listptr].c = Blistcol[listptr + 1].c;
		Blistcol[listptr].v = Blistcol[listptr + 1].v;
	}
...
//	popp();
	...
	//printbadlist();
	if (debug){
		cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ABptr=====" << ABptr << endl;
		cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!stackptr=====" << stackptr << endl;
		cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!blptr=====" << blptr << endl;
		cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!top=====" << top << endl;
	}
	while (ABptr > 0){
		randptr = generator() % ABptr + 1;
		...
		if (!found){
			...
			int errorcode = forcerror();
			if (errorcode==glerror){
				//insbadlist(r, c, v);
			//	delABlist(randptr);
				popp();
				return 0;
...
int  insgtable(int r, int c, int v){
		...
		v = glist[top].v = v;
		glist[top].zcnt = zcnt;
		if (debug){ if (gtable[r][c] != 0){ cout << "gtable collision r,c,v=" << r << c << v << endl; } }
	//	if (gtable[r][c] == 0){ gtable[r][c] = v; }
		gtable[r][c] = v;
		if (debug){ printglist(); }
		return 0;
	}
...
int  insg2table(int r, int c, int v){
	...
	++currg2cnt;
	r = g2list[currg2cnt].r = r;
	c = g2list[currg2cnt].c = c;
	v = g2list[currg2cnt].v = v;
	g2list[currg2cnt].zcnt = zcnt;
	g2table[r][c] = v;
	return 0;
}
//==========================================================
...
int inspuzzle(int r, int c, int v){
	...
	int res = 0;
	checkfin();
	if (thisisaguess){ nonguesscounter = 0; }
	else{
		++nonguesscounter;
		if (debug){ cout << "NONGUESSCOUNTER= " << nonguesscounter << endl; }
	}
...
int _tmain(){ //for windows
	...
	if (debug){
		...
		//	cout << "zcnt=" << zcnt << "savezcnt=" << savezcnt << endl;
		//	cout << "lzcnt=" << lzcnt << endl;
	}
	lzcnt = zcnt;
...
//	gseed = chrono::duration_cast<chrono::microseconds>(chrono::steady_clock::now().time_since_epoch()).count();
	...
	int mcnt = 0;
	while (true){
		if (stackptr >= 20){ printpuzzle(); exit(0); }
		tzcnt = zcnt;
		insertrow();
		mcnt = tzcnt - zcnt;
		procp();
...
int insertrow(){
	thisisaguess = true;
	printpuzzle();
	if (debug){ cout << "STACKPTR=" << stackptr << endl; }
	if ((zcnt < 46) && (nonguesscounter < 3)&&(nonguesscounter>0)){
		if (debug){ cout << "NONGUESSCOUNTER TOO LOW POPPING STACK" << endl; }
		popp();
		printpuzzle();
	}
	...
	for (int i = 1; i <= 9; i++){
		updp();
		if ((row[i].done == false) && (i != glastrow)){
			mcnt = gm(zrow, i);
			if (mcnt <= lowmcntrow){
				lowrow = i;
				lowmcntrow = mcnt;
	...
	//if ((zcnt<50)&&(donecnt > 2)){ popp(); popp(); }
	if (debug){ cout << "lowrow=" << lowrow << endl; }
	if (debug){ cout << "lowmcntrow=" << lowmcntrow << endl; }
	//updp();
	int r = lowrow;
	if (lowmcntrow < 5){ glastrow = r; }
	if (debug){
		cout << "MISSING NUMBERS" << endl;
		for (int i = 1; i <= lowmcntrow; i++){
			cout << mrow[i];
		}
		cout << endl;
		cout << "BLANK LOCATIONS" << endl;
		for (int i = 1; i <= lowmcntrow; i++){
			cout << blrow[i];
		}
		cout << endl;
	...
	pushp();
	int savestackptr = stackptr;

	int  c, v;

	while (true){
		mcnt = gm(zrow, r);
		int blcnt = gbls(zrow, r);
		int randindex = generator() % mcnt + 1;
		v = mrow[randindex];
		c = blrow[generator() %blcnt + 1];

		if (debug){ cout << "RCV=" << r << c << v << endl; }
		...
		if (glerr){
			...
			//	stackptr = savestackptr;
				fillerrorcounter++;
				if (debug){ cout << "FILLERRORCOUNTER=" << fillerrorcounter << endl; }
				if (fillerrorcounter > 8){
					if (debug){ cout << "FILL ERROR COUNTER  TOO HIGH POPPING STACK" << endl; }
				//	stackptr = savestackptr;
				//	popp();
					popp();
					savestackptr=stackptr;
...
void checkreinit(){
	checkfin();
	if (zcnt < lzcnt){ return; }
	lzcnt = zcnt;
	int res = procrowlists();
	if (res > 0){ return; }
	res = proccollists();
	if (res > 0){ return; }
	if (debug){ cout << "both lists empty and not done.  Popping puzzle stack." << endl; }
	cleargtable();
	clearbtable();
	stackptr = 0;
	popp();
	...
	for (int runit = 1; runit <= 3; runit++){
		for (int num = 1; num <= 9; num++){
			int res = collectpairsrow(runit, num);
		}
	}
	//================================================
	for (int cunit = 1; cunit <= 3; cunit++){
		for (int n = 1; n <= 9; n++){
			int res = collectpairscol(cunit, n);
		}
	}
	if (debug){
		cout << "======list of 50% chance A choices for row inserts=======" << endl;
		printAlistrow();
		cout << "==================================================================" << endl;
		cout << "======list of 50% chance B choices for row inserts=======" << endl;
		printBlistrow();
		cout << "===================================================================" << endl;

		cout << "======list of 50% chance A choices for col inserts=======" << endl;
		printAlistcol();
		cout << "==================================================================" << endl;

		cout << "======list of 50% chance B choices for col inserts========" << endl;
		printBlistcol();
		cout << "==================================================================" << endl;
	}
	return;
