...
int const rowsize = 10;
int const colsize = 10;
int const boxsize = 10;
int const numbersize = 10;
int const unitsize = 4;
...
int procrowlists(){
	...
	int const A = 1;
	int res = 0;
	int randomAorBpick = generator() % 2 + 1;
	if (randomAorBpick == A){
		 res = pushArow();
	}
	else{ res = pushBrow(); }
...
int inspuzzle(int r, int c, int v){
	...
	int res = 0;
	checkfin();
	if ((thisisaguess) && (zcnt > 30) && ((gtable[r][c]==v) || (btable[r][c] ==v))){ 
		if (debug){
			cout << "REJECTED"<<endl;}
		return 0; }
...
int getlowrow(int startrow){
	...
	if (lowmcntrow == 9){ exit(0); }
	if (row[lowrow].done == true){ exit(0); }
	glastrow = lowrow;
	glastrow = 0;
	return lowrow;
}
...
int pickrow(){
	...
	while (true){
		...
		if (rand1 < 1){ exit(0); }
		if (rand1 >mcnt){ exit(0); }
		if (rand2 < 1){ exit(0); }
		if (rand2 > mcnt){ exit(0); }
...
bool rowmatch(int r){
	...
	if (!newrowisduplicate){
		...
		lzcnt = zcnt;
		procpuzzle();
		if ((!glerr)&&(zcnt<lzcnt)){ pushp(); }
		else{  popp(); newrowisduplicate = true; }//to fake out main 
	}
...
int  insArow(int r, int c, int v){
	...
	v = Arow[ptrArow].v = v;
	if (debug){
		cout << "INSERTED AROW[PTRAROW]" << endl;
		pArow(ptrArow);
	}
	return 0;
}
...
void  pArow(int listptr){
	...
	cout << "CURRENT PTRAROW =" << ptrArow << endl;
	return;
}
//====================================================================
void  pAcol(int ptrA){
...
void  pBrow(int listptr){
	...
	cout << "CURRENT PTRBROW =" << ptrBrow << endl;
