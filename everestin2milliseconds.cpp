...
int insertc2choices(int cunit){
	...
	for (int n = 1; n <= 9; ++n){
		if ((colunit[cunit].nc[n] == 2) && (colunit[cunit].targcol[n] != 0) && (colunit[cunit].targbox[n] != 0)){
			...
			int places = 3;
			numinr1 = finrow(r1, n);
			if (numinr1){ r1occupied = true; --places; }
			else{ if (puzzle[r1][c] > 0){ --places; r1occupied = true; } }
			numinr2 = finrow(r2, n);
			if (numinr2){ r2occupied = true; --places; }
			else{ if (puzzle[r2][c] > 0){ --places; r2occupied = true; } }
			numinr3 = finrow(r3, n);
			if (numinr3){ r3occupied = true; --places; }
			else{ if (puzzle[r3][c] > 0){ --places; r3occupied = true; } }

			if (places == 2){
				c2ch[c].num[n].chAnum = n;
				c2ch[c].num[n].chBnum = n;
				int chAtried = c2ch[c].num[n].chAtried;
...
int pickrowchoice(int r){
	...
	for (int v = 1; v <= 9; v++){
		...
		chA = r2ch[r].num[v].chA;
		chB = r2ch[r].num[v].chB;
		chAnum = r2ch[r].num[v].chAnum;
		chBnum = r2ch[r].num[v].chBnum;
		chAtried = r2ch[r].num[v].chAtried;
		...
		//	int ABptr = generator() % 2;
		int tcol = 0;
		if ((chAtried==yes) && (chBtried==yes)){ continue; }
		//if ((chAnum == 0) && (chBnum == 0)){ continue; }
		...
		if (chAtried==no){
			...
			if (debug){
				cout << "*********************pickrowchoice A*******************" << endl;
				cout << "**r,v,chA,chB,chAnum,chBnum,chAtried,chBtried******************" <<r<<v<< chA << chB << chAnum << chBnum << chAtried << chBtried << endl;
			}
				if (puzzle[r][chA] == v){ continue; }
			if (puzzle[r][chB] == v){ continue; }
			if (puzzle[r][chA] != 0){
				if (debug){ cout << "**********************************************puzzle corrupted - reloading******chA*************************" << endl; }
				rldsaved();
			}
		}
		else{
		...
		int foundincol = fincol(tcol, v);
		if (foundincol){ continue; }
		int tbox = gboxfromrowandcol(r, tcol);
	    ...
	    if (inbox){ continue; }
			...
			if (tcol == chA){
				glastwrite = "*********************pickrowchoice A*******************";
				r2ch[r].num[v].chAtried = yes;
				r2ch[r].num[v].chAnum = v;
			}
			else{
				glastwrite = "*********************pickrowchoice B*******************";
				r2ch[r].num[v].chBtried = yes;
				r2ch[r].num[v].chBnum = v;
			}
			inserted = inspuzzle(r, tcol, v);
			return inserted;
...
void reinsertglist(){
	...
	int yes = 1;
	int inserted = no;
	if (currgcnt == 0){ return; }
	//if (currgcnt != oldcurrgcnt){ return; }
	 
	int listptr = generator() % currgcnt + 1;
...
//		printgbtables();
	...
	//if (zcnt > s2zcnt[2]){
		...
		if (lastwrite[2].valid){
			if (currgcnt > 0){
				//we may or may not be responsible. should we reload our saved copy or not?
				//if we are partially guilty, would that cgse a loop of bad reloads??
				if (s2zcnt[2] > 0){
